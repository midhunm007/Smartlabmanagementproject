...
@app.route('/labcycles/<labId>')
def get_lab_cycles(labId):
    # Fetch lab cycles associated with the specified labId from the database
    cursor.execute('''
        SELECT cyid
        FROM cycle
        WHERE lid = %s
    ''', (labId,))
    lab_cycles = cursor.fetchall()

    # Extract lab cycle IDs from the fetched data
    lab_cycle_ids = [cycle[0] for cycle in lab_cycles]

    # Assuming lab cycle IDs are strings, you can directly return them as JSON
    return jsonify({'lab_cycles': lab_cycle_ids})
